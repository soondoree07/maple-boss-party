...
> **보안 진행 중:** config.js를 신규 Publishable 키(`sb_publishable_…`)로 교체·푸시 `5abc320`. 다음: 배포 확인 → Supabase에서 **Disable legacy API keys**(노출 service_role+legacy anon 폐기). publishable=anon 역할 동일이라 RLS/RPC grant 그대로.
