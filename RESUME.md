# 메이플 보스 파티 기록 — 진행 상황 (2026-06-01 KST 기준)

## ★ 2026-06-01 — 월별 사이드 헤더: 인당 결정석 → 전리품 총합 (배포 완료)
- 오른쪽 월별 누적 사이드바(`js/monthly.js`) 각 월 카드의 **빨간 헤더 수치**(`.month-card-perhead`)를 "인당 총 결정석" → **"전리품 총합"**으로 변경.
- 계산식: 기존 `Σ(회차 결정석 ÷ 회차 참여자 수)`(인당) → **그 달 전리품 `price`를 1/N 없이 원가 그대로 합산**(`lootTotal`). null/NaN price는 0 처리·제외, 0이면 `0억`.
- 결정석 전용 import 3개(`getBoss`/`getEffectiveCrystal`/`resolveDifficultyKey`)와 `defaults`/`partyCount` 제거. 헤더 `title` 툴팁 "전리품 총합".
- **메인 상단 수익 카드(`earnings.js`)는 무변경** — 멤버별 실수익(결정석+분배 전리품, 인당)은 그대로. 월별 사이드 = 전리품 총 원가 / 메인 상단 = 인당 실수익으로 의미 분리(사용자 확인).
- 커밋 `1e568d5`, Vercel 자동 배포. node --check 통과, tree clean.
...
## ★ 2026-05-17 (후속3) — 남은 네이티브 알림 → 토스트/인라인/다이얼로그 (배포 완료)
